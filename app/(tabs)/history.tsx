...
import { calculateSavings } from '../../utils/publicTransportFares';
...
export default function HistoryScreen(): React.ReactElement {
  ...
  const totalSaved = completedPassengerTrips.reduce((acc, t) => {
    const savings = calculateSavings(t.from_location, t.to_location, t.cost_per_seat);
    return acc + (savings?.savedVsBus ?? 0);
  }, 0);

  const currency = trips.find((t) => t.role === 'passenger')?.currency ?? 'EUR';
  const totalKm  = completedPassengerTrips.length * 150; // rough estimate
  const co2Saved = Math.round(totalKm * 170 / 1000);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContent} testID="history-screen">
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((trip) => {
          const savings = calculateSavings(trip.from_location, trip.to_location, trip.cost_per_seat);
          ...
          return (
            <TouchableOpacity key={`${trip.role}-${trip.id}`} style={styles.tripCard} onPress={() => router.push(`/ride/${trip.id}`)} accessibilityRole="button" testID={`trip-item-${trip.id}`}>
              ...
              <View style={styles.tripMeta}>
                ...
                {savings && trip.status === 'completed' && trip.role === 'passenger' && (
                  <Text style={styles.savings} testID={`savings-${trip.id}`}>
                    Saved {formatCurrency(savings.savedVsBus, 'EUR')} vs bus
                  </Text>
                )}
              </View>
            </TouchableOpacity>
