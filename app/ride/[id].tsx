  ...
  const fetchRide = useCallback(async () => {
    ...
    try {
      const { data, error: dbErr } = await supabase
        .from('rides')
        .select('*, driver:users!driver_id(full_name, profiles(university, vehicle_details))')
        .eq('id', id)
        .single();

      if (dbErr || !data) { setError('Ride not found.'); return; }

      const driverObj  = data.driver as Record<string, unknown> | null;
      const profile    = (driverObj?.profiles as Record<string, unknown>[] | null)?.[0] ?? null;
      const vehicleRaw = profile?.vehicle_details as Record<string, unknown> | null;
      ...
      if (data.driver_id) {
        const { data: vData } = await supabase
          ...
          .eq('user_id', data.driver_id).single();
      ...
      setRide({
        ...
        driver: {
          full_name:   (driverObj?.full_name as string | undefined) ?? 'Driver',
