...
describe('calculateRideCost — cap enforcement', () => {
  it('per-seat cost × seats never exceeds total cost', () => {
    for (const seats of [1, 2, 3, 4, 5, 6, 7]) {
      const r = calculateRideCost(100, seats, 'ROI');
      expect(r.perSeatCost * seats).toBeLessThanOrEqual(r.totalCost + 0.01); // allow 1c rounding
    }
  });

  it('perSeatCost * seats ≤ totalCost for NI driver', () => {
    for (const seats of [1, 2, 3, 4, 5, 6, 7]) {
      const r = calculateRideCost(150, seats, 'NI');
      expect(r.perSeatCost * seats).toBeLessThanOrEqual(r.totalCost + 0.01);
    }
  });
});
...
describe('isWithinCap', () => {
  ...
  it('returns true exactly at the cap', () => {
    // totalCost = 43; maxPerSeat = 43/3 = 14.33; 3 × 14.33 = 42.99 ≤ 43
    expect(isWithinCap(14.33, 3, 100, 'ROI')).toBe(true);
  });
});
