...
## 8. CodeRabbit review debt — 4 of 6 stacked PRs triaged, #33–34 remain (updated 22 Jul, overnight run 2)

**Why:** PR #28 (`feat/full-sweep`) grew to 184 files — over CodeRabbit's 100-file review cap — so it was split into 6 stacked PRs along the existing commit history's natural boundaries: #29 (80 files, ✅ triaged), #30 (44 files, ✅ triaged), #31 (83 files, ✅ triaged), #32 (38 files, ✅ triaged), #33 (54), #34 (26, top of the branch).

**Status:** #29–#32 are all fully triaged — see PROGRESS.md for the detailed rundown of what was fixed vs. dismissed on each, and the PR comment threads for the full reasoning. #32's review actually landed at 23:57:09 UTC 21 Jul with 26 actionable comments (posted as one review-body summary rather than per-line inline comments this time — worth knowing the format varies). Fixed 10 real findings (a notify trigger that silently missed resubmissions, orphaned-file cleanup, a Maps key fallback bug, an account-deletion cache-residue gap, a live-tracking-token URL leak, and more) and dismissed 16 with reasons on the PR thread — two of those dismissals reference an already-known false-positive pattern (`type`→`interface` on a Supabase table row breaks `Record<string, unknown>` assignability) and an already-tracked legal item (Article 9 selfie-matching, explicitly deferred per Jordan's instruction). The fix was cherry-picked onto both `stack/04-driver-alert-fix` (for the PR) and `feat/full-sweep` (so tonight's session branch has it too) — this surfaced a real gap: `stack/04` predates the "Block 3 safe-area audit" work that added a `react-native-safe-area-context` Jest mock, so my new safe-area-aware fix in `DateTimeField.tsx` needed that mock pulled forward too, or its own test (and `SearchScreen`'s, which renders it) would fail with "No safe area value available."

**What to do:** #33 was triggered at 01:24 UTC 22 Jul and came back rate-limited (misleadingly worded "Full review finished... more reviews in 19 minutes" — this ack text is NOT reliable on its own; the giveaway is the "more reviews available in N minutes" line, which only appears when rate-limited). A retry is scheduled for ~01:45 UTC; once #33's real review lands, triage it the same way, then move to #34. Triage per the usual rule: fix real issues, dismiss noise with a one-line reason, merge only once every PR in the stack is clean (base branches first, since each depends on the one before it) — **and only when you explicitly say to merge; that step is never automatic.**
