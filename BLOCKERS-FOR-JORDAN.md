...
**What to do:** #32 was triggered and came back rate-limited ("Next review available in 26 minutes" as of 22 Jul) — will re-trigger once that window's passed, then move to #33, #34 the same way, one at a time, waiting for each to actually post findings (checking the summary comment's explicit rate-limit/processing state directly, not just the trigger acknowledgment, which can be misleading). Triage per the usual rule: fix real issues, dismiss noise with a one-line reason, merge only once every PR in the stack is clean (base branches first, since each depends on the one before it) — **and only when you explicitly say to merge; that step is never automatic.**
